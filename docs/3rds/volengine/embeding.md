...
注意
【重要】instructions 字段的配置直接决定模型推理效果。为了显著提升向量表示的精度，您需要根据具体的业务场景来定制该指令。请勿直接使用系统默认值。详情请参考设置 instructions 字段（推荐）。
使用说明
声明
...
第二步：获取向量
定义一个函数将单个文本或图片转换为向量表示。支持两种输入类型：文本和图片 URL。调用doubao-embedding-vision-251215模型获取float格式向量，再转换为numpy数组并展平为一维向量。
