...
- Python Pydantic Structured Output 便捷层方案已建立：[impls/python/pydantic-structured-output.CN.md](impls/python/pydantic-structured-output.CN.md)。
...
- Python core API family expansion：
  ...
  - 扩展 user-executed function tool spec，支持 function/custom 两种输入形态，并暂缓 provider-hosted/remote tool 抽象。
  - 扩展 API family capability，同时保留 v0.2 兼容字段。
- OpenAI function tool 协议映射。
- OpenAI custom tool 映射：
  - `ToolSpec(type="custom")` 映射为 OpenAI custom tool。
  - `custom_tool_call` 映射为 `FunctionCallItem(type="custom", input=...)`。
  - `FunctionResultItem(tool_type="custom")` 映射为 `custom_tool_call_output`。
  - streaming 复用 tool call events，并用 `metadata["tool_type"]` 标记 custom。
- Python Pydantic structured output helper：
  - `pydantic_output()` 从 Pydantic v2 type 生成 `ResponseFormat`。
  - 解析最终 `GenerationResponse` 为 typed object。
  - OpenAI client 提供 `generate_parsed()` / `agenerate_parsed()` convenience。
  - 不调用 provider SDK parse shortcut，不恢复 JSON mode。
