...
- 工具必须表达执行责任，区分 user-executed function tools 与 provider-hosted tools。
...
provider-side state 通过 `RemoteContextHint` 或 `provider_options` 的明确字段表达，仅作为优化提示。
...
- `HostedToolSpec` 或 provider-hosted tool base model。
- `MCPToolSpec` 可作为后续扩展，先保留 provider_options 也可接受。
...
- 明确 `ToolExecutionOwner.PROVIDER` 或 `REMOTE`。
