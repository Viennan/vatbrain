...
`vatbrain` 仍坚持 Full-context First：一次 generation 调用的语义事实来源是用户传入的完整 `Item` 序列。provider 的 `previous_response_id`、conversation、cache、stored response 或 context API 可以作为性能、成本和延迟优化，但不能成为核心状态模型。
...
`vatbrain` 不自动执行用户定义的函数工具，这一原则不变。但 provider-hosted tools 与 user-executed function tools 的执行责任不同，不能只用一个无差别 `ToolSpec` 表达。

例如火山方舟 Responses API 支持 function、web search、image process、knowledge search 和 MCP。function tool 由用户代码执行并回填结果；web search 或 image process 由 provider 执行；MCP 可能由 provider 托管远端调用。工具抽象必须明确执行责任，避免隐藏 orchestration。
...
- `core.tools`：表达 user-executed tools、provider-hosted tools、remote/MCP tools 和执行责任。
...
这些字段只是优化提示。用户仍应传入完整语义上下文。adapter 可以选择忽略 hint，也可以在 capability 中声明支持。
...
- 更新高层设计，明确 provider state、resource/file、hosted tools、media generation、reasoning visibility。
...
不改变。`previous_response_id` 是 provider-side optimization。`vatbrain` 的语义事实仍来自用户传入的完整上下文。adapter 可以使用 `previous_response_id` 降低成本或延迟，但不能让它成为不可见状态依赖。
...
不违反，前提是工具执行责任被显式表达。`vatbrain` 不自动执行用户函数；provider-hosted tools 是 provider 原生能力，用户显式声明后由 provider 执行。adapter 必须让调用方看见这种差异。
...
不建议直接开写完整 adapter。应先稳定 core 边界，随后做 Volcengine adapter MVP：Responses API、Files API、多模态 embedding。图片/视频生成和 hosted tools 可以作为下一阶段。
