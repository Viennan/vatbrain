...
```text
OpenAIClient(config).generate(model=..., items=...)
OpenAIClient(config).stream_generate(model=..., items=...)
OpenAIClient(config).embed(model=..., inputs=...)
```
...
其中 `items` 是完整上下文序列。`reasoning` 与 `tool_call_config` 表达跨厂商的通用 generation 语义。`provider_options` 只用于承载少量不可移植但必要的厂商专有参数。
