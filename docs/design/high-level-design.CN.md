...
`vatbrain` 可以允许用户显式传入远端上下文 hint，例如 previous response id、cache policy、store policy 或 provider conversation id。adapter 可以利用这些 hint 降低成本或延迟，但调用语义仍应由用户传入的完整 `Item` 序列定义。
...
工具声明必须表达执行责任。用户自定义 function tool 由用户代码执行并回填结果；provider-hosted tool 由 provider 执行；remote/MCP tool 可能由 provider 或外部服务执行。

`vatbrain` 不自动执行用户工具，也不隐藏 provider-hosted tool 的执行语义。
...
对于 provider-hosted tools，例如 web search、image process、knowledge search 或 MCP，`vatbrain` 可以提供通用或 provider-specific 声明模型，但必须明确这些工具由 provider 或远端服务执行。
...
- resource/file API。
- provider-hosted tools 与工具执行责任模型。
...
不改变。previous response、stored response、conversation、context cache 等能力只是 provider-side optimization。`vatbrain` 的语义事实仍应来自用户传入的完整上下文序列。
...
不违反，前提是工具执行责任被显式表达。用户自定义 function tool 仍由用户代码执行；provider-hosted tool 由 provider 执行；remote/MCP tool 由远端服务或 provider 托管执行。
